...
The query requires a `user(id: ID!)` field. Adjust the fields and arguments to match your API’s schema.

<img :src="withBase('/http-graphql.png')" alt="GraphQL query, variables, and response in the HTTP editor">
...
The existing priority applies: collection/folder defaults are overridden by the active environment, then Session. Pre-request scripts can update execution variables. Substitution happens before the variables JSON is parsed, so strings must remain valid JSON after substitution. Use GraphQL variables instead of interpolating input directly into the GraphQL document.
...
The client sends a JSON envelope with `query`, `variables` and an optional `operationName`. Default headers are `Content-Type: application/json` and `Accept: application/graphql-response+json, application/json;q=0.9`; explicit enabled headers take precedence. GraphQL requests default to not following redirects. Enter the intended endpoint explicitly and review any transport override.

Request code previews generate the wire JSON envelope. As with other HTTP requests, previews substitute environment values, but do not execute scripts or resolve Session values. Invalid GraphQL drafts do not produce code previews.
...
- Postman GraphQL bodies are imported into the GraphQL editor, including variables and operation name when present. Bruno OpenCollection GraphQL bodies currently import as JSON with a warning; review them before switching to GraphQL mode.
