...
## Наблюдаемый fixture ассистента

На Transport доступен изолированный журнал по ID прогона (`[A-Za-z0-9_-]`, 1–64 символа):

- `GET /qa/<runId>/state` возвращает `{runId, requests}`. Чтение не увеличивает счётчик; неизвестный ID возвращает пустой журнал.
- `POST /qa/<runId>/reset` очищает только этот журнал; новые запросы получают sequence с 1.
- `/qa/<runId>/echo?delay=0` принимает HTTP-запрос, записывает sequence, method, path, body и state; отвечает JSON со статусом 200. `delay` — 0–10 000 мс. Запись появляется до задержки ответа, поэтому потерянный ответ не означает отсутствие dispatch.

Журнал содержит максимум 100 запросов на ID и 20 ID на слушатель (дальше 429). Тело ограничено 16 KiB (413 с записью `rejected`); не используйте реальные данные или секреты. Заголовки не сохраняются. Состояния `receiving`, `received`, `rejected`, `aborted` отражают приём запроса, а не доставку ответа клиенту. После reset старый отложенный ответ не восстанавливает журнал. Разные слушатели имеют разные журналы; глобальный `/reset` не заменяет reset конкретного QA ID.

Перед проверкой подтверждения сбросьте журнал, после подготовки preview проверьте ноль записей, после принятия — одну запись с ожидаемыми методом/телом. Повтор клика/IPC не должен добавлять вторую. Этот fixture доказывает dispatch; число выполнений пользовательского скрипта проверяется отдельно по runtime-результату/инструментированному тесту, а не по демонстрационному `/metrics`.

## Scoped-команды проверки
