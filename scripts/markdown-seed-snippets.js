...
function printHelp() {
  console.log(`
Fake snippets seed for markdown storage.

Usage:
  node scripts/markdown-seed-snippets.js [options]
...
function buildFragmentBody(options, profile, snippetIndex, fragmentIndex) {
  ...
  return [
    '// Generated by markdown-seed-snippets.js',
...
async function main() {
  ...
  if (engineResponse.engine !== 'markdown') {
    throw new Error(
      `Current storage engine is "${engineResponse.engine}". Switch to "markdown" before seeding.`,
    )
  }
