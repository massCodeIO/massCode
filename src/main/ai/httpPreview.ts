...
import { resolveHttpCollection } from '../http/collection'
import { readHttpSession } from '../http/runtime/session'
import { scriptsTrusted } from '../http/scripts/trust'
...
export function httpRequestPreview(
  ...
  const session
    = options.includeSession !== false
      ? readHttpSession(String(vaultIdentity()), payload.environmentId)
      : { variables: {}, names: [] }
