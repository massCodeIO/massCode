...
class MermaidWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    root.className = 'my-3 overflow-auto rounded-md border border-border p-4'
...
export function createMermaidBlocks(options: MermaidBlocksOptions = {}) {
  const { enabled = true, isDark = false } = options

  return StateField.define<DecorationSet>({
    create(state) {
      return buildDecorations(state, enabled, isDark)
    },
    update(decorations, transaction) {
      if (transaction.docChanged) {
        return buildDecorations(transaction.state, enabled, isDark)
      }
